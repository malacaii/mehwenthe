getgenv().Toggy = {
 Toggled = true, -- Toggle Duh
 ShiftAmount = 1000000000, -- amount of cash until game shutdown
 BreakTime = 100000, -- amount of times it devlivers pizzas until it takes a 120 second break
 Status = true, -- Shows a small status of what it is doing
 SecondsToWait = 1, -- Time To wait
 AntiStink = true, -- Client Particles get disabled
 LowRender = true, -- Lowers Fps Good for farmming overnight and to run in background recommended
}
loadstring(game:HttpGet('https://raw.githubusercontent.com/malacaii/mehwenthe/main/yay.lua'))()
